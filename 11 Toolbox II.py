...
def toolWindow():
    ...
    input = cmds.textFieldGrp(parent=mainCol, text='Sequential Renamer input')
    
    cmds.text('Toolbox II:')
    cmds.button(label='Build Joint Chain from Selected CV Curve', command='BuildJointChainFromSelection()')
    cmds.button(label='Show/Hide Local Rotation Axes', command='ShowHideLocalRotationAxes()')
    cmds.button(label='Switch Unlocked Influences', command='SwitchUnlockedInfluences()')
